(function(){
    ...
    (function(){
        ...
        function __init__(self, message, filename, line, col, pos, is_eof) {
            var self = this;
            self.stack = new Error().stack;
            self.message = message;
            self.line = line;
    ...
    (function(){
        ...
        COMPILER_VERSION = "c0b6e5f9a3c2a83bde3e6dc627755bb0e4b31899";
        NATIVE_CLASSES = {
            "Image": {},
            "RegExp": {},
            "Error": {},
        ...
        function parse($TEXT, options) {
            ...
            class_ = function() {
                var name, externaldecorator, class_details, definition, i, class_var_names, visitor, stmt;
            ...
            object_ = embed_tokens(function() {
                var a, first, start, type, name, quoted;
                expect("{");
                first = true;
                a = [];
                while (!is_("punc", "}")) {
                    ...
                    if (first && !is_token(peek(), "punc", ":")) {
                        return dict_comprehension(a);
                    }
                    first = false;
                    start = S.token;
                    if (!is_object_key()) {
                        token_error(start, "Expecting an object key here");
                    }
                    type = start.type;
                    name = start.value;
                    quoted = start.type === "string";
                    next();
            ...
            function is_object_key() {
                var tmp_;
                tmp_ = S.token.type;
                return tmp_ === "num" || tmp_ === "string" || tmp_ === "name" || tmp_ === "operator" || tmp_ === "keyword" || tmp_ === "atom";
            }
            ...
            function dict_comprehension(a) {
                var _$rapyd$_Unpack, start, left, right;
                if (a.length) {
                    _$rapyd$_Unpack = [a[0].start, a[0].value];
                    start = _$rapyd$_Unpack[0];
                    right = _$rapyd$_Unpack[1];
                    if (start.type === "operator" || start.type === "keyword") {
                        token_error(start, "invalid syntax, dict comprehension cannot have an operator or keyword as a key");
                    }
                    left = token_as_atom_node(start);
                } else {
                    left = expression(false);
                    if (!is_("punc", ":")) {
            ...
            subscripts = function(expr, allow_calls) {
                var start, is_slice, slice_bounds, str_, c, funcname, tmp_, args, ret;
                start = expr.start;
                if (is_("punc", ".")) {
                    next();
            ...
            maybe_conditional = function(no_in) {
                var start, expr, pystyle, ne, yes, conditional;
                start = S.token;
                expr = expr_ops(no_in);
